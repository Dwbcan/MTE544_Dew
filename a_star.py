...
def search(maze, start, end):
    ...
    # Helper: choose heuristic. Supports 'euclidean' and 'manhattan'.
    def _heuristic(a, b, method='euclidean'):
    ...
    # Default to Euclidean heuristic; can be changed by passing a fourth argument to search
    heuristic_method = 'euclidean'

    start_node = Node(None, start)
    start_node.g = 0.0     # cost from start Node
    start_node.h = _heuristic(start_node.position, end, heuristic_method)     # heuristic estimated cost to end Node
    start_node.f = start_node.g + start_node.h

    end_node = Node(None, end)
    end_node.g = float('inf')       # set a large value if not defined
    end_node.h = 0.0       # heuristic estimated cost to end Node (0 at goal)
    ...
    while len(yet_to_visit_dict) > 0:
        ...
        for child in children:
            ...
            # g is the cost from start to this child (add Euclidean distance from parent to child)
            dx = child.position[0] - current_node.position[0]
            dy = child.position[1] - current_node.position[1]
            step_cost = sqrt(dx * dx + dy * dy)
            child.g = current_node.g + step_cost
            # Heuristic costs calculated here
            child.h = _heuristic(child.position, end_node.position, heuristic_method)
